...
void qapi_event_send_device_tray_moved(const char *device, const char *id, bool tray_open)
{
    ...
    emit(QAPI_EVENT_DEVICE_TRAY_MOVED, qmp);
...
void qapi_event_send_pr_manager_status_changed(const char *id, bool connected)
{
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_PR_MANAGER_STATUS_CHANGED, qmp);
...
void qapi_event_send_quorum_failure(const char *reference, int64_t sector_num, int64_t sectors_count)
{
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_QUORUM_FAILURE, qmp);
...
void qapi_event_send_quorum_report_bad(QuorumOpType type, bool has_error, const char *error, const char *node_name, int64_t sector_num, int64_t sectors_count)
{
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_QUORUM_REPORT_BAD, qmp);
