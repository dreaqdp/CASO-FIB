...
void qapi_event_send_balloon_change(int64_t actual)
{
    ...
    emit(QAPI_EVENT_BALLOON_CHANGE, qmp);
...
void qapi_event_send_device_deleted(bool has_device, const char *device, const char *path)
{
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_DEVICE_DELETED, qmp);
...
void qapi_event_send_dump_completed(DumpQueryResult *result, bool has_error, const char *error)
{
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_DUMP_COMPLETED, qmp);
...
void qapi_event_send_mem_unplug_error(const char *device, const char *msg)
{
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_MEM_UNPLUG_ERROR, qmp);
...
void qapi_event_send_rtc_change(int64_t offset)
{
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_RTC_CHANGE, qmp);

    visit_free(v);
    qobject_unref(qmp);
}

void qapi_event_send_command_dropped(QObject *id, CommandDropReason reason)
{
    QDict *qmp;
    QMPEventFuncEmit emit;
    QObject *obj;
    ...
    visit_start_struct(v, "COMMAND_DROPPED", NULL, 0, &error_abort);
    visit_type_q_obj_COMMAND_DROPPED_arg_members(v, &param, &error_abort);
    visit_check_struct(v, &error_abort);
    visit_end_struct(v, NULL);

    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_COMMAND_DROPPED, qmp);
