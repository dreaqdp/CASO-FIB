...
extern TraceEvent _TRACE_AMDVI_IR_GENERATE_MSI_MESSAGE_EVENT;
extern TraceEvent _TRACE_AMDVI_IR_IRTE_GA_EVENT;
...
extern uint16_t _TRACE_AMDVI_IR_GENERATE_MSI_MESSAGE_DSTATE;
extern uint16_t _TRACE_AMDVI_IR_IRTE_GA_DSTATE;
...
#define TRACE_AMDVI_IR_GENERATE_MSI_MESSAGE_ENABLED 1
#define TRACE_AMDVI_IR_IRTE_GA_ENABLED 1
...
#define TRACE_AMDVI_IR_GENERATE_MSI_MESSAGE_BACKEND_DSTATE() ( \
    trace_event_get_state_dynamic_by_id(TRACE_AMDVI_IR_GENERATE_MSI_MESSAGE) || \
    false)

static inline void _nocheck__trace_amdvi_ir_generate_msi_message(uint8_t vector, uint8_t delivery_mode, uint8_t dest_mode, uint8_t dest, uint8_t rh)
{
    if (trace_event_get_state(TRACE_AMDVI_IR_GENERATE_MSI_MESSAGE) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:amdvi_ir_generate_msi_message " "vector %d delivery-mode %d dest-mode %d dest-id %d rh %d" "\n",
                 getpid(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , vector, delivery_mode, dest_mode, dest, rh);
    }
}

static inline void trace_amdvi_ir_generate_msi_message(uint8_t vector, uint8_t delivery_mode, uint8_t dest_mode, uint8_t dest, uint8_t rh)
{
    if (true) {
        _nocheck__trace_amdvi_ir_generate_msi_message(vector, delivery_mode, dest_mode, dest, rh);
    }
}

#define TRACE_AMDVI_IR_IRTE_GA_BACKEND_DSTATE() ( \
    trace_event_get_state_dynamic_by_id(TRACE_AMDVI_IR_IRTE_GA) || \
    false)

static inline void _nocheck__trace_amdvi_ir_irte_ga(uint64_t addr, uint64_t data)
{
    if (trace_event_get_state(TRACE_AMDVI_IR_IRTE_GA) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:amdvi_ir_irte_ga " "addr 0x%"PRIx64" offset 0x%"PRIx64 "\n",
                 getpid(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , addr, data);
    }
...
static inline void trace_amdvi_ir_irte_ga(uint64_t addr, uint64_t data)
{
    if (true) {
        _nocheck__trace_amdvi_ir_irte_ga(addr, data);
    }
}
