...
extern TraceEvent _TRACE_TZ_MSC_IRQ_ENABLE_EVENT;
...
extern uint16_t _TRACE_TZ_MSC_IRQ_ENABLE_DSTATE;
...
#define TRACE_TZ_MSC_IRQ_ENABLE_ENABLED 1
...
#define TRACE_TZ_MSC_IRQ_ENABLE_BACKEND_DSTATE() ( \
    trace_event_get_state_dynamic_by_id(TRACE_TZ_MSC_IRQ_ENABLE) || \
    false)

static inline void _nocheck__trace_tz_msc_irq_enable(int level)
{
    if (trace_event_get_state(TRACE_TZ_MSC_IRQ_ENABLE) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:tz_msc_irq_enable " "TZ MSC: int_enable = %d" "\n",
                 getpid(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , level);
    }
...
static inline void trace_tz_msc_irq_enable(int level)
{
    if (true) {
        _nocheck__trace_tz_msc_irq_enable(level);
    }
}
